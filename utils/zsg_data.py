...
class myDataset(Dataset):
    def __init__(self,data_root=DATA_ROOT,data_type="train1"):
        self.data_root = data_root
        self.data_type = data_type
        self.image_dir = osp.join(self.data_root,self.data_type)
        self.image_paths = []
        self.bboxes = []
        self.phrases = []

        self.anno_csv_path = osp.join(
            self.image_dir, "label.csv"
        )
        with open(self.anno_csv_path,encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                self.image_paths.append(row["image_name"])
                self.bboxes.append("["+",".join([row["bbox_x"],row["bbox_y"],
                str(int(row["bbox_x"])+int(row["bbox_width"])),
                str(int(row["bbox_y"])+int(row["bbox_height"]))])+"]")
                self.phrases.append(row["label_name"])
    
    def __len__(self):
        return len(self.image_paths)

    def __getitem__(self,idx):
        image_path = osp.join(self.image_dir, self.image_paths[idx])
        image = np.array(Image.open(image_path).convert("RGB"))
        bbox = ast.literal_eval(self.bboxes[idx])
        phrases = self.phrases[idx]
        #print(phrases)
        data = {"image": image, "bbox": bbox, "phrases": phrases, "path": image_path}
        return data
