...
class SLICViT(nn.Module):
    def __init__(self, model='vit14', alpha=0.8, n_segments=[10, 50, 100, 200],
        ...
        self.sigma = sigma
        self.textadapter = ClipAdapter(768)
        self.imgadapter = ClipAdapter(768)
    ...
    def get_mask_scores(self, im, text):
        ...
        image_features = self.imgadapter(image_features.float())
        image_features = image_features.permute(0, 2, 1)

        with torch.no_grad():
            text = clip.tokenize([text]).cuda()
            text_features = self.model.encode_text(text)
        text_features.requires_grad_()
        text_features = self.textadapter(text_features.float())
        print("alpha ",self.textadapter.alpha)
        ...
        logits = (image_features * text_features.unsqueeze(-1)).sum(1).to(torch.float64)[0]
        #assert logits.size(0) == 1
        
        # return masks.cpu().numpy(), logits.cpu().float().detach().numpy()[0]
        return masks, logits
    ...
    def forward(self, im, text, train, **args):
        ...
        h, w = im.shape[:2]
        if train == 1:
            heatmap = self.get_heatmap(im, text, normalize=False)
            return heatmap
        else:
            with torch.no_grad():
                heatmap = self.get_heatmap(im, text, normalize=True)
                bbox = self.box_from_heatmap(heatmap.cpu().numpy())
                bbox[:, ::2] = bbox[:, ::2] * w / 224.
